      ...
      <div className="flex rounded-md overflow-hidden">
        ...
        <div className="relative flex-1 rounded-r-md overflow-hidden">
          ...
          <input
            type="text"
            className={cn(
              'border-0 focus:ring-0 rounded-r-md outline-none focus:outline-none px-3 w-full relative z-0',
              (isFocused || hasValue) ? 'pt-7 pb-2' : 'py-3'
            )}
            onFocus={handleFocus}
            onBlur={handleBlur}
          ...
          >
            Preço {required && <span className="text-red-500">*</span>}
          </label>
...
// Componente para seleção de prestadores de serviço
...
export default function EditProductPage() {
  ...
  const [selectedServiceProviders, setSelectedServiceProviders] = useState<string[]>([]); // IDs dos prestadores de serviço
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [loading, setLoading] = useState(true);
  const [validationError, setValidationError] = useState('');
  
  // Carregar dados do produto quando o componente é montado
  useEffect(() => {
    const loadProductData = async () => {
      if (!productId) {
        console.error('EditProductPage: No product ID provided');
        toast({
          title: "Erro",
      ...
      // Verificar se os dados do produto foram passados via state
      if (!productData) {
        console.error('EditProductPage: No product data provided');
        toast({
          title: "Erro",
      ...
      try {
        setLoading(true);
        console.log(`EditProductPage: Loading product data for ID: ${productId}`);
        console.log('EditProductPage: Product data received:', productData);
        
        // Preencher os campos com os dados do produto
        setProductName(productData.productName || '');
        setProductDescription(productData.productDescription || '');
        setProductPrice(productData.productPrice ? productData.productPrice.toString() : '');
        
        // Determinar o tipo do produto baseado nos prestadores de serviço
        const hasServiceProviders = productData.serviceProviderCalendars && productData.serviceProviderCalendars.length > 0;
        setProductType(hasServiceProviders ? 'service' : 'product');
        
        // Mapear os calendários dos prestadores de serviço
        const serviceProviderIds = productData.serviceProviderCalendars?.map(
          calendar => calendar.serviceProviderCalendarId
        ) || [];
        setSelectedServiceProviders(serviceProviderIds);
        
        console.log('EditProductPage: Form fields populated successfully');
        
      } catch (error) {
        console.error('EditProductPage: Error loading product:', error);
        toast({
          title: "Erro ao carregar produto",
          description: error instanceof Error ? error.message : "Erro desconhecido ao carregar o produto",
          variant: "destructive",
        });
    ...
    loadProductData();
  }, [productId, productData, toast, navigate]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const productData = {
        ...
        serviceProviderCalendarIds: productType === 'service' ? selectedServiceProviders : []
      };
      
      // Chamar API para atualizar produto
