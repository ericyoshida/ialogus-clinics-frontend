...
import { CreateClinicDto, editClinic } from '@/services/clinics'
...
export default function EditClinicPage() {
  ...
  // Capturar dados da clínica a ser editada
  const [clinicId, setClinicId] = useState<string>('');
  const [referrer, setReferrer] = useState('');
  
  useEffect(() => {
    // Obter dados da clínica do state ou localStorage
    const stateData = location.state;
    let clinicData = null;
    let clinicIdFromState = '';
    
    if (stateData?.editMode && stateData?.clinicData) {
      // Dados vindos do state
      clinicData = stateData.clinicData;
      clinicIdFromState = stateData.clinicId;
    } else {
      // Tentar obter do localStorage
      const savedData = localStorage.getItem('temp_edit_clinic');
      if (savedData) {
        const parsedData = JSON.parse(savedData);
        clinicData = parsedData;
        clinicIdFromState = parsedData.id;
      }
    }
    
    if (clinicData && clinicIdFromState) {
      setClinicId(clinicIdFromState);
      setFormData({
    ...
    // Obter referrer da state do location
    const from = location.state?.from || '/dashboard/agents/create/clinic';
    setReferrer(from);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (validateForm()) {
      try {
        ...
        // Chamar a API para editar a clínica
        await editClinic(clinicId, clinicData);
        
        // Limpar dados temporários do localStorage
        localStorage.removeItem('temp_edit_clinic');
        
        // Mostrar mensagem de sucesso
        toast({
          title: "Clínica editada com sucesso!",
          description: `A clínica ${formData.name} foi atualizada com sucesso.`,
        });
        ...
        toast({
          ...
          description: "Ocorreu um erro ao editar a clínica. Tente novamente.",
          variant: "destructive",
        });
      } finally {
    ...
    <div className="max-w-7xl -mt-4 px-2 sm:px-3 lg:px-4 pb-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-5 pl-1">
        <div className="flex-1 mb-2 sm:mb-0">
          ...
          <p className="text-gray-500 text-sm">Atualize as informações da clínica</p>
        </div>
      </div>
