...
import { CreateCompanyDto, editCompany } from '@/services/companies'
...
export default function EditCompanyPage() {
  ...
  // Capturar dados da empresa a ser editada
  const [companyId, setCompanyId] = useState<string>('');
  const [referrer, setReferrer] = useState('');
  
  useEffect(() => {
    // Obter dados da empresa do state ou localStorage
    const stateData = location.state;
    let companyData = null;
    let companyIdFromState = '';
    
    if (stateData?.editMode && stateData?.companyData) {
      // Dados vindos do state
      companyData = stateData.companyData;
      companyIdFromState = stateData.companyId;
    } else {
      // Tentar obter do localStorage
      const savedData = localStorage.getItem('temp_edit_company');
      if (savedData) {
        const parsedData = JSON.parse(savedData);
        companyData = parsedData;
        companyIdFromState = parsedData.id;
      }
    }
    
    if (companyData && companyIdFromState) {
      setCompanyId(companyIdFromState);
      setFormData({
        name: companyData.name || '',
        shortName: companyData.shortName || '',
        brandDescription: companyData.brandDescription || '',
    ...
    const from = location.state?.from || '/dashboard/agents/create/company';
    setReferrer(from);
    ...
    if (validateForm()) {
      try {
        ...
        toast({
          ...
          description: "Ocorreu um erro ao editar a empresa. Tente novamente.",
          variant: "destructive",
        });
      } finally {
    ...
    <div className="max-w-7xl -mt-4 px-2 sm:px-3 lg:px-4 pb-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-5 pl-1">
        <div className="flex-1 mb-2 sm:mb-0">
          ...
          <p className="text-gray-500 text-sm">Atualize as informações da empresa</p>
        </div>
      </div>
